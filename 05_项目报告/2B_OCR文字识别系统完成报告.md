...
综合检测 Demo 已经实现了“同一张图中同时识别二维码/条形码和 OCR 文字”，但混合场景下 OCR 精度不如单独文字识别稳定，主要原因如下：

1. 同一张图里既有码又有文字，干扰更多。二维码/条码是强黑白块状结构，OCR 又要识别旁边的中文、英文和数字。综合图中的边框、二维码、文字区域、阴影和压缩痕迹都会互相干扰。

2. Tesseract 对中文复杂场景的鲁棒性有限。它识别英文和数字通常更稳定，但在中文、表格、小票、身份证样式、中英文混合场景下，容易出现相近字误识别或漏识别。

3. 当前综合样张是生成图片，不是真实扫描件。字体、字号、抗锯齿和缩放过程会改变笔画边缘，中文笔画又比英文复杂，因此更容易被 OCR 误判。

4. 当前 OCR 是整图识别，没有先做文字区域检测。更理想的流程应该是：

```text
先定位文字区域 -> 裁剪文字区域 -> 单独 OCR -> 合并读码与文字结果
```

当前 Demo 直接对整张图进行 OCR，二维码、边框和标题都会进入 OCR 输入区域，因此会影响文字识别结果。

5. 综合 Demo 更偏功能展示，而不是工业级高精度识别系统。它主要证明系统已经能够完成“读码 + OCR + 结构化字段提取 + CSV/Excel 导出”的完整流程，但还没有加入复杂版面分析和精细文字检测。

后续如果继续提升精度，可以从以下方向改进：

- 在 OCR 前先检测并遮挡二维码/条形码区域，减少码图案对 OCR 的干扰；
- 对文字区域进行裁剪，只识别右侧或指定文本区域；
- 引入 PaddleOCR 或 EasyOCR 这类带文字检测能力的 OCR 引擎，提升中文和复杂版面识别效果；
- 针对身份证、票据、表格等场景设计固定模板或区域规则；
- 将综合检测流程拆成“码区域检测、文字区域检测、OCR、结构化字段解析、结果融合”几个独立模块，便于后续优化。
